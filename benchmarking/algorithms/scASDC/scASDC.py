...
sys.path.append(
    "/home/kohl/gits/ClemensKohl/CAdir_benchmarking/algorithms/scASDC/utils/"
)
import argparse
import numpy as np
...
def train_scASDC(model, dataset, X_raw, sf):
    ...
    for epoch in range(200):
        ...
        x_bar, q, pred, _, h, A_pred, h1, meanbatch, dispbatch, pibatch, zinb_loss = (
            model(data, adj)
        )
        ...
        re_graphloss = F.kl_div(
            A_pred.view(-1).log(), adj.to_dense().view(-1), reduction="batchmean"
        )
...
if __name__ == "__main__":
    ...
    args.pretrain_path = "{path}/{name}.pkl".format(path=args.outdir, name=args.name)

    print(args)
    x, y = prepro("{dataset}".format(dataset=args.dataset))
    ...
    count = adata.X
    # y = adata.obs_keys
    construct_graph(count, y, "ncos", name=args.name, outdir=args.outdir, topk=top_k)
    ...
    train_scASDC(model, dataset, count, sf)
    with torch.no_grad():
        out = model.eval(dataset)
    outf = "{outdir}/pred.txt".format(outdir=args.outdir)
    with open(outf) as f:
        for x in out:
            f.write(x)
