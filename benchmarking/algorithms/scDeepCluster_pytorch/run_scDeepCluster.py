...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gamma", default=1.0, type=float, help="coefficient of clustering loss"
    )
    parser.add_argument(
        "--sigma", default=2.5, type=float, help="coefficient of random noise"
    )
    ...
    adata = normalize(
        adata, size_factors=True, normalize_input=True, logtrans_input=True
    )
    ...
    if args.n_clusters > 0:
        ...
        pretrain_latent = (
            model.encodeBatch(torch.tensor(adata.X, dtype=torch.float64)).cpu().numpy()
        )
        ...
        Group = pd.Series(
            y_pred_init, index=np.arange(0, adata_latent.n_obs), name="Group"
        )
