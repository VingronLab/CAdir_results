...
log_file <- file.path(
  LOG_DIR,
  paste0("gpu_memory_torch_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log")
)

cat("==============================================\n", file = log_file)
cat(
  "GPU Memory Monitoring Log (PyTorch Version)\n",
  file = log_file,
  append = TRUE
)
...
tryCatch(
  ...
  error = function(e) {
    message(
      "Warning: Failed to activate conda environment. Please check CONDA_ENV_NAME."
    )
  }
)
...
genecell_c <- function(x1) {
  if (ncol(x1) < 6 || nrow(x1) < 10) {
    ...
    if (is.null(cell_names)) {
      cell_names <- paste0("cell_", seq_len(ncol(x1)))
    }
  ...
  if (inherits(x1, "dgCMatrix") && !has_bad(x1)) {
    ...
    x1_clean <- x1[
      !apply(x1_dense_for_check, 1, function(row) any(!is.finite(row))),
      ,
      drop = FALSE
    ]
  }
  ...
  for (a in unique_clust) {
    ...
    if (is.null(dim(rm11_g)) || nrow(rm11_g) < 2 || ncol(rm11_g) < 6) {
      next
    }
    ...
    if (is.numeric(new)) {
      df <- data.frame(
        v1 = colnames(rm11_g),
        v2 = rep(as.integer(new), ncol(rm11_g))
      )
  ...
  if (k == 1 || nrow(clust1) <= 6) {
    ...
    for (b in 1:k) {
      ...
      if (ncol(sub_matrix) >= 6 && nrow(sub_matrix) >= 20) {
        ...
        sub_clust <- data.frame(
          v1 = colnames(sub_matrix),
          v2 = rep(1L, ncol(sub_matrix))
        )
        data[[paste0(
          a,
          ":",
          paste0("(", paste(new_path, collapse = "->"), ")")
        )]] <- sub_clust
      }
    }
  }
...
if (file.exists(INPUT_FILE)) {
  ...
  if (is.null(rownames(matrix1))) {
    rownames(matrix1) <- paste0("g", seq_len(nrow(matrix1)))
  }
  if (is.null(colnames(matrix1))) {
    colnames(matrix1) <- paste0("c", seq_len(ncol(matrix1)))
  }
  ...
  stop(
    "Error: Input file not found. Please check INPUT_FILE path in the configuration section."
  )
}
