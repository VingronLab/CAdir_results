...
parser.add_argument("--file", default="./", type=str, help="path to dataset")
parser.add_argument(
    "--method", default="ncos", type=str, help="graph construction method"
)
...
def construct_graph(features, label, method, topk, fname):
    ...
    if method == "heat":
        ...
        dist = np.dot(features, features.T) / (
            np.linalg.norm(y) * np.linalg.norm(features)
        )
...
fname_global = "{dir}/global_{mthd}.txt".format(dir=args.outdir, mthd=args.method)
A_global = construct_graph(x, y, args.method, topk_global, fname_global)

# Constructing an adjacency matrix of local features
# fname_local = "graph/LPS/lps_int2_2local_ncos.txt"
fname_local = "{dir}/local_{mthd}.txt".format(dir=args.outdir, mthd=args.method)

A_local = construct_graph(x, y, args.method, topk_local, fname_local)
