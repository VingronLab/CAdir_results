...
parser.add_argument("--ngenes", default="./", type=int)
parser.add_argument(
    "--ignore_noint", action=argparse.BooleanOptionalAction, default=False
)

args = parser.parse_args()

print("ignore_noint:", args.ignore_noint)
...
with h5py.File(file_path, "r") as file:
    ...
    print(labels.shape)
    print(
        "Number of variables (genes) in the original dataset:", data.shape[1]
    )  # Column: Genes
    print(
        "Number of observations (cells) in the original dataset:", data.shape[0]
    )  # Row: Cells

    # Normalization, selection of highly variable genes
    adata = normalize(
        adata,
        highly_genes=args.ngenes,
        filter_min_counts=False,
        ignore_noint=args.ignore_noint,
    )


# Save the normalized data to a new .h5 file
with h5py.File(save_path, "w") as f_normalized:
    f_normalized.create_dataset(
        "X", data=adata.X, compression="gzip", compression_opts=9
    )
    # 如果有标签或其他形式的附加信息，也保存
    f_normalized.create_dataset(
        "Y", data=adata.obs["labels"], compression="gzip", compression_opts=9
    )

    f_normalized.create_dataset(
        "highly_variable",
        data=adata.var["highly_variable"],
        compression="gzip",
        compression_opts=9,
    )
    f_normalized.create_dataset(
        "size_factors",
        data=adata.obs["size_factors"],
        compression="gzip",
        compression_opts=9,
    )
    f_normalized.create_dataset(
        "raw", data=adata.raw.X, compression="gzip", compression_opts=9
    )
