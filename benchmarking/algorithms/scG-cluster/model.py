...
class scG(tf.keras.Model):
    def __init__(
        ...
        self.encoder = EncoderG(
            X, G_adj_n, L_adj_n, hidden_dim=self.hidden_dim, latent_dim=self.latent_dim
        )
        self.decoderX = DecoderX(self.latent_dim, self.raw_dim, self.dec_dim)
        self.decoderA = DecoderA(self.adj_dim)

        self.cluster_layer = ClusteringLayer(n_clusters=n_clusters, name="clustering")
        ...
        for epoch in range(1, epochs + 1):
            with tf.GradientTape(persistent=True) as tape:
                ...
                zinb = ZINB(
                    pi=pi,
                    theta=disp,
                    scale_factor=self.size_factors,
                    ridge_lambda=0,
                    debug=False,
                )
                ...
                A_rec_loss = tf.reduce_mean(MSE(self.L_adj, A_out)) + tf.reduce_mean(
                    MSE(self.G_adj, A_out)
                )

                reg_loss = (
                    tf.reduce_sum(self.encoder.losses)
                    + tf.reduce_sum(self.decoderX.losses)
                    + tf.reduce_sum(self.decoderA.losses)
                )
            ...
            if loss < best_loss:
                ...
                try:
                    self.encoder.save_weights(
                        self.model_pth + "/pretrain_encoder.weights.h5"
                    )
                    self.decoderA.save_weights(
                        self.model_pth + "/pretrain_decoderA.weights.h5"
                    )
                    self.decoderX.save_weights(
                        self.model_pth + "/pretrain_decoderX.weights.h5"
                    )
    ...
    def train(
        ...
        self.cluster_layer.clusters.assign(
            tf.convert_to_tensor(centers, dtype=tf.float32)
        )
        ...
        for epoch in range(0, epochs):
            ...
            if epoch % n_update == 0:
                ...
                print(
                    "epoch=%d, ACC= %.4f, NMI= %.4f, nmi_arithmetic= %.4f, ARI= %.4f, SC=%.4f"
                    % (epoch, acc, nmi, nmi_arithmetic, ari, sc)
                )
                ...
                for i in range(self.cluster_layer.n_clusters):
                    ...
                    if len(indices) > 0:
                        new_centers[i, :] = tf.reduce_mean(
                            tf.gather(z, indices), axis=0
                        )
                self.cluster_layer.clusters.assign(
                    tf.convert_to_tensor(new_centers, dtype=tf.float32)
                )
            ...
            if stop_loss < best_loss:
                ...
                try:
                    self.encoder.save_weights(
                        self.model_pth + "/train_encoder.weights.h5"
                    )
                    self.decoderA.save_weights(
                        self.model_pth + "/train_decoderA.weights.h5"
                    )
                    self.decoderX.save_weights(
                        self.model_pth + "/train_decoderX.weights.h5"
                    )
