...
def get_args(
    ...
    parser.add_argument(
        "--dataset_path", default=dataset_path, type=str, help="path to dataset (adata)"
    )
    parser.add_argument("--pretrain_epochs", default=pretrain_epochs, type=int)
    parser.add_argument("--train_epochs", default=train_epochs, type=int)
    ...
    parser.add_argument("--n_clusters", default=n_clusters, type=int)
    parser.add_argument("--method", default="ncos", type=str)
...
if __name__ == "__main__":
    ...
    cluster_number = args.n_clusters
    ...
    # adjacency graph
    file_path = "{dir}/global_{mthd}.txt".format(dir=args.model_pth, mthd=args.method)
    ...
    # adjacency graph
    file_path = "{dir}/local_{mthd}.txt".format(dir=args.model_pth, mthd=args.method)
    edges = np.loadtxt(file_path, dtype=int)
    num_nodes = np.max(edges) + 1
    ...
    if y is not None:
        ...
        outfile = "{}/cluster_pred.txt".format(args.model_pth)
