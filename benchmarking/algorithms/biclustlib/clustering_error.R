...
bic2list <- function(biclusters) {
  ...
  for (i in seq_len(biclusters@Number)) {
    bic_list[[i]] <- list(
      as.integer((which(biclusters@RowxNumber[, i]) - 1)),
...
biclustlib_CE <- function(pred_biclusters, true_biclusters) {
  ...
  ord_r <- order(match(
    rownames(pred_biclusters@RowxNumber),
    rownames(true_biclusters@RowxNumber)
  ))

  pred_biclusters@RowxNumber <- pred_biclusters@RowxNumber[
    ord_r,
    ,
    drop = FALSE
  ]
  ...
  ord_c <- order(match(
    colnames(pred_biclusters@NumberxCol),
    colnames(true_biclusters@NumberxCol)
  ))

  pred_biclusters@NumberxCol <- pred_biclusters@NumberxCol[,
    ord_c,
    drop = FALSE
  ]
...
biclustlib_RNIA <- function(pred_biclusters, true_biclusters) {
  ...
  RNIA <- relative_non_intersecting_area(
    pred_biclist,
    true_biclist,
    nrows,
    ncols
