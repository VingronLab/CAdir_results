# TODO: Rewrite the sim_eval script as a simpler eval.R with only necessary
...
gene_stats <- function(splatter_sim, grp2clust) {
  ...
  for (i in seq_len(length(groups))) {
    ...
    plots_list[[i]] <- list(
      "de_genes" = plot_de_genes(splatter_sim, grp, cluster),
