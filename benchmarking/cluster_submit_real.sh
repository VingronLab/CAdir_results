#!/bin/bash
#TODO: Update with new parameters from simulated benchmarking.
...
files="/project/kohl_data/CAdir/data/real/preprocessed/${dataset}_filtered.rds"
          ...
          for snn in "${SNN_mode[@]}"; do
            for gc in "${calc_gckNN[@]}"; do
              ...
              for ov in "${overlap[@]}"; do
                ...
                tmp_sh="${here_dir}/bench_real_${dataset}_${nm}.sh"
cat << EOF > "$tmp_sh"
#!/bin/bash
...
chmod +x "$tmp_sh"
                        ...
                        mxqsub --stdout="${logdir}/bench_real_${dataset}_${nm}.stdout.log" \
                               --group-name="bench_real_${dataset}_${filename}_${algorithm}" \
                               --threads=$THREADS \
                               --memory="$MEM_caclust" \
                               -t $MINUTES \
                               bash "$tmp_sh"
...
# end cameans
    ...
    nm="${algorithm}_${filename}_ntop-${nt}_kdirs-${k}_ndim-${d}_qcut-${q}_angle-${a}_aplquant-${p}"
    tmp_sh="${here_dir}/bench_real_${dataset}_${nm}.sh"
