...
option_list <- list(
  ...
  make_option(
    ...
    action = "store",
    default = FALSE,
    help = "Generic option to toggle if you want to run a test",
    metavar = "logical"
  ),
...
# Set the angle to NULL if the input is 0.
# This will use the Salpha cutoff for CAdir.
if (angle == 0) {
  angle <- NULL
}
...
if (isTRUE(is_divbiclust)) {
  ...
  # calculate the dropout rates in the input data set
  # (the data that has been preprocessed)
