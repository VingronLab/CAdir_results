...
if (is(sce, 'Seurat')) {
  sce <- SingleCellExperiment(
    list(counts = GetAssayData(sce, slot = 'count', assay = 'RNA')),
...
ggsave(
  ...
  file = file.path(
    imgdir,
    paste(name, "total_counts_violinplot.png", sep = "_")
  )
)
...
ggsave(
  p_detected,
  file = file.path(imgdir, paste(name, "detected_violinplot.png", sep = "_"))
)
...
ggsave(
  ...
  file = file.path(
    imgdir,
    paste(name, "percent_mito_violinplot.png", sep = "_")
  )
)


p_sumVSdetected <- plotColData(
  sce,
  x = "sum",
  y = "detected",
  colour_by = "discard"
...
ggsave(
  ...
  file = file.path(
    imgdir,
    paste(name, "sum_vs_detected_scatter.png", sep = "_")
  )
)
...
ggsave(
  p_hist_detected,
  file = file.path(imgdir, paste(name, "detected_hist.png", sep = "_"))
)
...
ggsave(
  p_hist_sum,
  file = file.path(imgdir, paste(name, "sum_hist.png", sep = "_"))
)
...
if (!is.null(mt_perc)) {
  stopifnot(is.numeric(mt_perc))
  mt_keep <- which(sce$subsets_Mito_percent < mt_perc)
  sce <- sce[, mt_keep]
}
...
ggsave(
  p_umap,
  file = file.path(imgdir, paste(name, "Truth_umap.png", sep = "_"))
)
...
ggsave(
  p_umap_dblscore,
  file = file.path(imgdir, paste(name, "doublet_score_umap.png", sep = "_"))
)

p_umap_dblclass <- plotUMAP(sce, colour_by = "scDblFinder.class")
ggsave(
  p_umap_dblclass,
  file = file.path(imgdir, paste(name, "doublet_class_umap.png", sep = "_"))
)
