...
ggsave(
  plot = p_expr,
  file = file.path(imgdir, "cluster_6_top_genes.pdf"),
  width = 2200,
  height = 2000,
  units = "px"
)
...
#### Correct outliers
```{r}
cell_rnk <- get_top_cells(cadir = cak, caobj = ca, cluster = "cluster_6")
...
p_cl5_cells <- cluster_apl(
  ...
  cak,
  cluster = "cluster_6",
  direction = cak@directions["cluster_6", ],
  group = which(cak@cell_clusters == "cluster_6"),
...
ggsave(
  plot = p_cl5_cells,
  file = file.path(imgdir, "cluster_6_cells.pdf"),
  width = 1600,
  height = 1600,
  units = "px"
)
...
sce_sank <- sce_corr[,sce_corr$cadir == "cluster_6"]
...
ggsave(plot = sank,
       filename = file.path(imgdir, "sankey_plot_cluster_6.pdf"),
       width = 2700,
       height = 2000,
       units = "px")
```
