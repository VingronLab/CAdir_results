...
p_bt4 <- cluster_apl(
  ...
  show_genes = FALSE,
  label_genes = FALSE,
  show_lines = c("T_cell", "Macrophage"),
  size_factor = 0.1,
  point_size = 2
) +
...
# grp_idx <- which(
#   cak@cell_clusters == base::sample(unique(cak@cell_clusters), 1)
# )
#
...
# cak@parameters$sa_cutoff <- cutoff
cak@parameters$sa_cutoff <- CAdir:::deg2rad(55)
...
p_bt5 <- cluster_apl(
  ...
  guides(shape = FALSE, size = FALSE, linetype = FALSE) +
  ggtitle("Inferred cutoff angle line - no genes")
...
p_bt6 <- cluster_apl(
  ...
  guides(shape = FALSE, size = FALSE, linetype = FALSE) +
  ggtitle("Inferred cutoff angle line - with genes")
...
ggsave(
  plot = p_angle,
  filename = file.path(imgdir, "cutoff_angle.png"),
  width = 3200,
  height = 1000,
  units = "px"
)
...
ggsave(
  plot = p_angle,
  filename = file.path(imgdir, "cutoff_angle.tiff"),
  width = 3200,
  height = 1000,
  units = "px"
)
...
ggsave(
  plot = fig,
  filename = file.path(imgdir, "new_vis_panel.tiff"),
  width = 3200,
  height = 1600,
  units = "px"
)
...
fig1 <- ((p_bt4 + p_bt3) / (p_bt1 + p_bt2) / (p_bt5 + p_bt6)) +
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(size = 18))
...
ggsave(
  plot = fig1,
  filename = file.path(imgdir, "plotting_functions.tiff"),
  width = 3200,
  height = 2400,
  units = "px"
)
