...
info <- left_join(data.frame("Biclusters" = names(c5cols), "col" = c5cols), info, by = "Biclusters")
...
wrap <- 17
df_cl$node <- factor(stringr::str_wrap(df_cl$node, wrap), levels = stringr::str_wrap(order, wrap))
df_cl$next_node <- factor(stringr::str_wrap(df_cl$next_node, wrap), levels = stringr::str_wrap(order, wrap))

names(c5cols) <- stringr::str_wrap(names(c5cols), wrap)


sank <- ggplot(df_cl, aes(
  x = x,
  next_x = next_x,
  node = node,
  next_node = next_node,
...
p_gs_enr <- ggplot(gr, aes(x = cluster, y = gene_set, fill = -log10(padj))) +
  ...
  coord_fixed() +
  theme_bw() +
  labs(x = "Cluster", y = "Gene Set", title = "Gene Set enrichment", fill =expression(atop("-log"[10], "\npadj"))) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "black"),
...
fig3 <- wrap_elements(wrap_plots(pcl_panel, ncol = 3)) +
  ...
  free(p_gs_enr + theme(plot.title = element_text(hjust = 0.5)), type = "label", side = "tbr") +
