...
# Show that B and T cells are close
...
p_bt <- cluster_apl(
  ...
  ggplot2::geom_abline(
    ...
    linetype = "dashed",
    size = 1) +
geom_label(data=as.data.frame(t_dir*3), aes(x, y), label="T_cell", color = "#d44a3d", vjust=-1) +
  ...
  ggplot2::geom_abline(
    ...
    linetype = "dashed",
    size = 1) +
geom_label(data=as.data.frame(i_dir*4), aes(x, y), label="Immune_cell", color = "#00b1ea", vjust=-1)

p_bt

ggsave(
  plot = p_bt,
  file = file.path(imgdir, "B_cell_cluster.pdf"),
  width = "1600",
  height = "1600"
)
```
...
# Show that endothelial cell populations are close
...
p_ecs <- cluster_apl(
  ...
  ggplot2::geom_abline(
    ...
    linetype = "dashed",
    size = 1) +
geom_label(data=as.data.frame(e_dir*2), aes(x, y), label="Endothelial_cell", color = "#d44a3d", hjust=-1)

p_ecs

ggsave(
  plot = p_ecs,
  file = file.path(imgdir, "endothelial_cell_clusters.pdf"),
  width = "1600",
  height = "1600"
)
```
...
p <- ggplot(long_ang, aes(x=cluster, y = to_cluster, fill = angle)) +
  geom_tile() +
  scale_fill_gradient_mpimg(name = "green", trans = "reverse") +
  theme_bw()+
