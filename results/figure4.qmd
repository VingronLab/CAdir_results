...
old_col <- any(grepl("^ARI_cells$", colnames(df_all)))
if (isTRUE(old_col)) {
  sel <- is.na(df_all$ARI) & !is.na(df_all$ARI_cells)
  df_all$ARI[sel] <- df_all$ARI_cells[sel]
}
...
old_col <- any(grepl("^ARI_cells$", colnames(df)))
if (isTRUE(old_col)) {
  sel <- is.na(df$ARI) & !is.na(df$ARI_cells)
  df$ARI[sel] <- df$ARI_cells[sel]
}
```
...
df_tile <- df_real %>%
  ...
  group_by(dataset) %>%
  mutate(
    ismax = (round(Max_ARI, 2) == max(round(pick(Max_ARI), 2), na.rm = TRUE))
  ) %>%
  mutate(
    is2nd = (round(Max_ARI, 2) ==
      round(
...
p <- ggplot(df_tile, aes(x = algorithm, y = dataset, fill = Max_ARI)) +
  ...
  labs(
    x = "Algorithm",
    y = "Dataset",
    title = "Maximum ARI on experimental data"
  )
...
ggsave(
  plot = p,
  filename = file.path(imgdir, "real_bench_max_ARI_cells.png"),
  width = 2000,
  height = 1750,
  units = "px"
)
...
```{r}
# NOTE: For figure4 panel.
p_tile <- ggplot(df_tile, aes(x = algorithm, y = dataset, fill = Max_ARI)) +
  geom_tile() +
  geom_tile(
    data = df_tile[df_tile$ismax, ],
    aes(x = algorithm, y = dataset),
...
```{r}
df_tile_mean <- df_real %>%
  select(algorithm, dataset, Mean_ARI) %>%
  distinct() %>%
  group_by(dataset) %>%
  mutate(
    ismax = (round(Mean_ARI, 2) == max(round(pick(Mean_ARI), 2), na.rm = TRUE))
  ) %>%
  mutate(
    is2nd = (round(Mean_ARI, 2) ==
      round(
        sort(Mean_ARI, decreasing = TRUE, na.last = NA)[2],
        2
      ))
  ) %>%
  ungroup() %>%
...
df_tile_mean$ismax[is.na(df_tile_mean$ismax)] <- FALSE
df_tile_mean$is2nd[is.na(df_tile_mean$is2nd)] <- FALSE
```
...
p_tile_mean <- ggplot(
  df_tile_mean,
  aes(x = algorithm, y = dataset, fill = Mean_ARI)
) +
  geom_tile() +
  geom_tile(
    data = df_tile_mean[df_tile_mean$ismax, ],
    aes(x = algorithm, y = dataset),
    fill = NA,
    color = "#d44a3d",
    linewidth = 1
  ) +
  # geom_tile(data = df_tile_mean[df_tile_mean$is2nd & !df_tile_mean$ismax, ],
  #   aes(x = algorithm, y = dataset),
  #   fill = NA,
  #   color = "#8c5fa8",
  #   size = 1) +
  geom_text(
    aes(label = round(Mean_ARI, 2)),
    color = "white",
    # size = 3,
    size = 12,
    size.unit = "pt",
  ...
  labs(
    x = "Algorithm",
    y = "Dataset",
    title = "Mean ARI on experimental data"
  )

ggsave(
  plot = p_tile_mean,
  filename = file.path(imgdir, "real_bench_mean_ARI_cells.png"),
  width = 2000,
  height = 1750,
  units = "px"
)
...
stats <- df %>%
  ...
  mutate(
    perc_ARI_succ = round((n_runs - NA_ARI) / n_param, 2),
...
p_succ <- stats %>%
  ...
  mutate(
    stat = factor(
      stat,
      levels = c("perc_ARI_succ", "perc_ARI_NA", "perc_crash")
    )
  ) %>%
...
ggsave(
  ...
  filename = file.path(
    imgdir,
    "barplots_fraction_successful_runs_ARI_wide.pdf"
  ),
  width = 8,
  height = 4.2
)
ggsave(
  ...
  filename = file.path(
    imgdir,
    "barplots_fraction_successful_runs_ARI_wide.png"
  ),
  width = 8,
  height = 4.2
)
...
mpgPink <- "#d95276" # Rich Pink
mpgBrown <- "#a25b43" # Warm Brown
mpgCyan <- "#00CED1"

eval_files <- list.files(
  file.path("./results/benchmarking/results/scalability", date, "out"),
  pattern = "*.csv",
  full.names = T
)
...
scal_df$total_runtime <- scal_df$runtime +
  ifelse(
    is.na(scal_df$runtime_dimreduc),
    yes = 0,
    no = scal_df$runtime_dimreduc
  )
...
alph <- 0.8
rt <- ggplot(scal_df, aes(x = dataset, y = (total_runtime / 60))) +
  geom_point(
    aes(group = algorithm, color = algorithm, alpha = algorithm),
    size = 2
  ) +
  geom_line(
    aes(
  ...
  labs(
    x = "Number of cells",
    y = "Running time (min)",
    title = "Runtime vs Data Set Size"
  ) +
  scale_alpha_manual(
    values = c(
      "CAdir" = 1,
  ...
  coord_trans(x = "log10") +
  scale_y_log10(guide = guide_axis(n.dodge = 1)) +
  # scale_x_log10(guide = guide_axis(n.dodge = 1)) +
...
ggsave(
  plot = rt,
  filename = file.path(imgdir, "runtime_singleproc.png"),
  width = 2200,
  height = 1800,
  units = "px"
)
...
alph <- 0.8
rt_no_DL <- scal_df %>%
  filter(algorithm != "scDeepCluster") %>%
  filter(algorithm != "scG-cluster") %>%
  ggplot(aes(x = dataset, y = (total_runtime / 60))) +
  geom_point(
    aes(group = algorithm, color = algorithm, alpha = algorithm),
    size = 2
  ) +
...
ggsave(
  plot = rt_no_DL,
  filename = file.path(imgdir, "runtime_singleproc_no_DL.pdf"),
  width = 2200,
  height = 1800,
  units = "px"
)
ggsave(
  plot = rt_no_DL,
  filename = file.path(imgdir, "runtime_singleproc_no_DL.png"),
  width = 2200,
  height = 1800,
  units = "px"
)
...
# date = "260418_TEST"
date = "20260325_revision2"
indir <- "./results/benchmarking/results/ct_detection/"
if (
...
```{r}
source("./results/cluster_tabula_muris.R")
...
ggsave(
  plot = rt_suppl,
  filename = file.path(imgdir, "rt_suppl_panel.png"),
  width = 6000,
  height = 2069,
  units = "px"
)
...
ggsave(
  plot = rt_suppl,
  filename = file.path(imgdir, "rt_suppl_panel.tiff"),
  width = 6000,
  height = 2069,
  units = "px"
)
...
ggsave(
  plot = rt_suppl,
  filename = file.path(imgdir, "rt_suppl_panel.pdf"),
  width = 6400,
  height = 2200,
  units = "px"
)
...
ggsave(
  plot = fig1,
  filename = file.path(imgdir, "upd_figure4_panel.pdf"),
  width = 6400,
  height = 4400,
  units = "px"
)
...
ggsave(
  plot = fig1,
  filename = file.path(imgdir, "upd_figure4_panel.png"),
  width = 6400,
  height = 4400,
  units = "px"
)
...
ggsave(
  plot = fig1,
  filename = file.path(imgdir, "upd_figure4_panel.tiff"),
  width = 6400,
  height = 4400,
  units = "px"
)
...
ggsave(
  plot = fig4,
  filename = file.path(imgdir, "figure4_panel.tiff"),
  width = 5600,
  height = 2240,
  units = "px"
)
...
suppl_panel <- (((free(p_mean_ari, type = "label") +
  ...
  (p_tile_mean + p_succ + plot_layout(width = c(1, 1)))) +
  plot_annotation(tag_levels = "a") &
  theme(
    text = element_text(size = 20),
    plot.title = element_text(size = 22, face = "plain"),
...
ggsave(
  plot = suppl_panel,
  filename = file.path(imgdir, "suppl_mean_panel.tiff"),
  width = 6000,
  height = 4500,
  units = "px"
)
...
ggsave(
  plot = suppl_panel,
  filename = file.path(imgdir, "upd_suppl_mean_panel.pdf"),
  width = 6000,
  height = 2800,
  units = "px"
)
...
ggsave(
  plot = suppl_panel,
  filename = file.path(imgdir, "upd_suppl_mean_panel.png"),
  width = 6000,
  height = 2800,
  units = "px"
)
...
ggsave(
  plot = suppl_panel,
  filename = file.path(imgdir, "upd_suppl_mean_panel.tiff"),
  width = 6000,
  height = 2800,
  units = "px"
)
...
ggsave(
  plot = dct_panel,
  filename = file.path(imgdir, "ct_detection_panel.png"),
  width = 6000,
  height = 2069,
  units = "px"
)
...
ggsave(
  plot = dct_panel,
  filename = file.path(imgdir, "ct_detection_panel.tiff"),
  width = 6000,
  height = 2069,
  units = "px"
)
...
ggsave(
  plot = dct_panel,
  filename = file.path(imgdir, "ct_detection_panel.pdf"),
  width = 6400,
  height = 2200,
  units = "px"
)
