...
dfz <- zeisel_eval %>%
  ...
  mutate(simulation = factor(simulation, levels = c(
    "0.02_0.75_0.75",
    "0.06_0.75_0.75",
    "0.1_0.75_0.75",
    "0.02_1.5_1.5",
...
df_all$algorithm <- factor(df_all$algorithm,
  levels =
    c(
      "CAdir",
      "CAdir_auto",
...
p_succ <- stats %>%
  ...
  mutate(
    stat = factor(stat, levels = c("perc_ARI_succ", "perc_ARI_NA", "perc_crash"))
  ) %>%
  ...
  scale_fill_manual(
    values = c("perc_ARI_succ" = "#006c66", "perc_ARI_NA" = "#ef7c00", "perc_crash" = "#d44a3d"),
    name = "% runs",
...
ggsave(
  plot = p_succ,
  filename = file.path(imgdir, "barplots_fraction_successful_runs_ARI_wide.png"),
  width = 8, height = 4.2
)
p_succ
```
...
suppl_panel <- (
    (
      (free(p_mean_ari, type = "label") + theme(legend.position = "none") + p_mean_nmi) +
